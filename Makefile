...
KINECTLIB = libkinectinator.so
INIT_SCENEGRAPH = SpelchkCommon/SceneGraph.o

CXXOPTS = -O2 -g -Wall -fmessage-length=0
CXXDEFS = -DFREEGLUT_STATIC -DGLEW_STATIC
CXXINCS = -Iinclude -ISpelchkCommon -I$(CURDIR)/kinect/OpenNI/Include -I/usr/include/ni  

CXXFLAGS = $(CXXOPTS) $(CXXDEFS) $(CXXINCS)

LDLIBS = -L/usr/local/lib -lGLEW -lglut -lGL -lXmu -lX11 -lm -lSOIL -L$(CURDIR) -lkinectinator 
...
$(KINECTLIB): SpelchkCommon/KinectInator.cpp
	...
	g++ -shared -Wl,-soname,libkinectinator.so -o $(KINECTLIB) SpelchkCommon/KinectInator.o -lboost_system-mt -lboost_thread-mt -Lkinect/OpenNI/Lib -Lkinect/OpenNI/Samples/Bin/x64-Release -lOpenNI	
