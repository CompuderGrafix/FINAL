...
{
    // rs = ray.org - sphere.c
    vec3 rs = ray.org - s.c; // displacement/direction to ray origin from the sphere
    float B = dot(rs, ray.dir); // degree to which the ray direction and the currents ray are collinear.
    float C = dot(rs, rs) - (s.r * s.r); // squared magnitude of rs * the square of the radius
    float D = B * B - C;
